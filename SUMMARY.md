# Summary

* [概述](README.md)
* [接口规则](RULES.md)

## API

* [Common - 公共接口](api/common.md)
  * [SSO\_Login - 单点登录入口](api/common/sso_login.md)
  * [GetUserInfo - 获取用户信息](api/common/getuserinfo.md)
  * [MenuList - 获取系统菜单](api/common/menulist.md)
  * [FunctionList - 获取菜单功能列表](api/common/functionlist.md)
* [Wechat - 微信接口](api/wechat.md)
  * [GetPreAuthLink - 获取预授权地址](api/wechat/getpreauthlink.md)
  * [MpList - 获取公众号列表](api/wechat/mplist.md)
  * [GetMpMenu - 获取公众号自定菜单](api/wechat/getmpmenu.md)
  * [EditMpMenu - 修改公众号自定义菜单](api/wechat/editmpmenu.md)
  * [UnbindMP - 解绑公众号](api/wechat/unbindmp.md)
  * [GetAccessToken - 获取公众号AccessToken](api/wechat/getaccesstoken.md)
  * [GetComponentInfo - 获取开放平台授权信息](api/wechat/getcomponentinfo.md)
* [StatusCode - 状态码](api/statuscode.md)

